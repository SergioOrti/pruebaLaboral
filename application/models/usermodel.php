...
class UserModel extends CI_model {
	...
	function create_user($name, $email, $password, $telefono, $direccion) {
		try {
			$user = array(
				'name'     => $name,
				'email'    => $email,
	...
	function update_user($_id, $name, $email, $password, $telefono, $direccion) {
		try {
			$query = new MongoDB\Driver\BulkWrite();
			$query->update(['_id' => new MongoDB\BSON\ObjectId($_id)], ['$set' => array(
				'name'     => $name,
				'email'    => $email,
				'password' => $password,
				'telefono' => $telefono,
